export default {
  ...
  "verify.success": "アカウントが正常に確認されました。サインインできるようになりました。",
  "verify.error": "確認リンクが無効であるか、有効期限が切れています。",
  "verify.button.signin": "サインインへ進む",
  "verify.info.title": "アカウント確認",
  "verify.info.description": "メール確認が有効になっています。確認用リンクをメールアドレスに送信しました。リンクをクリックしてアカウントを有効化してください。",
  ...
  "admin.config.general.default-language.description": "これはすべてのユーザーに適用されます。各ユーザーはプロフィールで個別に言語をカスタマイズできます。",
  ...
  "admin.config.security.session-duration.description": "ユーザーが再度ログインするまでの時間（デフォルト: 3 ヶ月）。",
  "admin.config.appearance.logo": "ロゴ",
  "admin.config.appearance.logo.description": "新しい画像をアップロードしてロゴを変更できます。画像は、PNG形式でアスペクト比が1:1である必要があります。",
  "admin.config.appearance.logo-dark": "ダークモード用ロゴ",
  "admin.config.appearance.logo-dark.description": "ダークモード用の個別のロゴをアップロードします。画像は PNG 形式で、縦横比 1:1 である必要があります。",
  "admin.config.appearance.logo.placeholder": "画像を選択",
  "admin.config.security.custom-password-policy": "カスタムパスワードポリシー",
  "admin.config.security.custom-password-policy.description": "パスワードポリシーのカスタマイズを有効にするかどうか。無効にした場合、デフォルトの 8 文字以上のポリシーが使用されます。",
  ...
  "admin.config.email.share-download-notification-message.description": "受信者がファイルをダウンロードしたときに共有作成者に送信されるメッセージ。利用可能な変数:\n {recipientEmail} - 受信者のメールアドレス\n {fileName} - ダウンロードされたファイル名\n {shareUrl} - 共有の URL",
  "admin.config.security.enable-email-verification": "メール確認を有効にする",
  "admin.config.security.enable-email-verification.description": "ユーザーがサインインする前にメールアドレスの確認を必須にするかどうか。これは SMTP が有効な場合のみ有効にできます。",
  "admin.config.email.verification-subject": "確認メールの件名",
  "admin.config.email.verification-subject.description": "ユーザー登録時に送信されるメールの件名。",
  "admin.config.email.verification-message": "確認メッセージ",
  "admin.config.email.verification-message.description": "ユーザー登録時に送信されるメッセージ。{url} は確認用 URL に置き換えられます。",
  ...
  "admin.config.share.file-retention-period.description": "共有の期限切れまたは削除後にファイルを保持する期間。「すべての共有への管理者アクセスを許可」が有効な場合のみ有効です。ファイルを永続的に保持するには -1 に設定します。",
  ...
  "admin.config.s3.enabled.description": "ローカルファイルシステムの代わりに S3 を使用して共有ファイルを保存するかどうか。警告: ClamAV が有効な場合、スキャン対象のファイルは一時的に S3 からダウンロードされます。",
  ...
  "error.msg.email_already_exists": "このメールアドレスのアカウントは既に存在します。パスワードでサインインし、マイアカウントページで手動で {0} アカウントを連携してください。",
